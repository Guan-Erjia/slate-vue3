...
function resolveDescendants(children: any[]): Descendant[] {
  ...
  function addChild(child: Node | Token | string): void {
    if (child == null) {
      return;
    }
...
export function createSelection(
  ...
  const anchor: AnchorToken | undefined = children.find(
    (c) => c instanceof AnchorToken,
  );
  const focus: FocusToken | undefined = children.find(
    (c) => c instanceof FocusToken,
  );
