                ...
                <p>
                    <label>
                        ...
                        <select v-model="config.chunkSize">
                            <option v-for="chunkSize in chunkSizeOptions" :key="chunkSize" :value="chunkSize">
                                {{ chunkSize }}
                            </option>
                        </select>
                    </label>
            ...
            <p>
                <label>
                    ...
                    <select v-model="config.contentVisibilityMode">
                        ...
                        <option v-if="config.chunking && config.chunkDivs" value="chunk">Lowest chunks</option>
                    </select>
                </label>
            </p>
...
import { createEditor as slateCreateEditor, Descendant, Editor } from 'slate-vue3/core'
import {
    Editable,
    RenderElementProps,
    RenderChunkProps,
...
const lastLongAnimationFrameDuration =
    ref<number | null>(null)

const lastKeyPressDuration = computed<number | null>(() => keyPressDurations[0] ?? null)

const averageKeyPressDuration = computed(() => keyPressDurations.length === 10
...
const renderElement = ({ attributes, children, element }: RenderElementProps) => {
    const style: CSSProperties = {
        contentVisibility: config.contentVisibilityMode === 'element' ? 'auto' : undefined,
    }
    switch (element.type) {
        case 'heading-one':
            return h(Heading, {
...
const renderChunk = ({ attributes, children, lowest }: RenderChunkProps) => {
    const style: CSSProperties = {
        contentVisibility: config.contentVisibilityMode === 'chunk' && lowest ? 'auto' : undefined,
